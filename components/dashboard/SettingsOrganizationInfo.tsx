...
export default function SettingsOrganizationInfo() {
  return (
    <div className="min-w-0 rounded-2xl border border-[#232D47] bg-[#0A1121]/60 p-4">
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-[#232D47] bg-[#0A1128] px-4 py-3 text-white focus:border-[#1CEAEA] focus:outline-none focus:ring-1 focus:ring-[#1CEAEA]"
          />
        </div>
        ...
        {selects.map(({ label, value }) => (
          <div key={label}>
            ...
            <button
              type="button"
              className="flex w-full cursor-pointer items-center justify-between rounded-lg border border-[#232D47] bg-[#0A1128] px-4 py-3 text-left text-white hover:bg-white/5"
            >
              {value}
